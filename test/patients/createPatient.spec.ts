...
describe('Pruebas para el controlador de creación de pacientes', () => {
  ...
  test('Debe devolver 409 si el DNI está duplicado', async () => {
    await request(app).post('/patients').send(validPatient);
    const res = await request(app).post('/patients').send(validPatient);
  ...
  test('Debe devolver 400 si se le pasa un objeto mal formado (falta idenNumber)', async () => {
    ...
    expect(res.body.error).toEqual(
      'Patient validation failed: idenNumber: Path `idenNumber` is required.',
    );
  });
