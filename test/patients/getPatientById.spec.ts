...
describe('Pruebas para comprobar el funcionamiento del controlador getPatientById', () => {
  const paciente1 = {
  ...
  test('Debe devolver al paciente con el id que buscamos', async () => {
    const resForId = await Patient.find({ idenNumber: '48210567Z' });

    const res = await request(app).get(`/patients/${resForId[0]._id}`);

    expect(res.status).toBe(200);
    expect(res.body.name).toEqual('Juanma García');
  });
  ...
  test('Debe devolver un error 500 si simulamos un error en el server', async () => {
    const spy = vi
      .spyOn(Patient, 'findById')
      .mockRejectedValueOnce(new Error('Database error: Is down'));

    const res = await request(app).get('/patients/69f473e840f5b1f3cd7852f4');
