 ...
 * @returns - Retorna el staff buscado por id o el error que produzca la funcion findById.
 */
export async function modifyStaffById(id : string, staffData : Partial<iStaff>) {
    try {
        const result = await Staff.findByIdAndUpdate(id, {$set: staffData}, { new: true });
        return result;
    }
    catch (error) {
        return error;
